...
> Добавил `schemaId` в `responseFormat.payload` - по нему FE выбирает, каким рендерером показать ответ.

**Bad** (two disconnected thoughts, reads like two emails glued together):
> Добавил `schemaId` в `responseFormat.payload`.
>
> Напомню: `type: 1` в `responseFormat` - обычный markdown, `type: 2` - JSON. `schemaId` нужен только для `type: 2`.
...
- Good: `В графе агента:` / `В ответе /api/agents/execute:`
- Worse (instructional how-to tone): `Как выставить в графе агента:` / `Что приходит в ответе /api/agents/execute:`
...
- `widget-components-dsl@1` → рендерим в dashboard-режиме
- `markdown-components-dsl@1` → рендерим в markdown-режиме
- `schemaId` не задан → дефолтный рендерер
```
