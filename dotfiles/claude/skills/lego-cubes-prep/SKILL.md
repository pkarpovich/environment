...
Stages 1-5 are pre-recording (Obsidian note prep). Stage 6 is post-recording publishing (.mp4 to NAS with sidecars). Stage 7 is post-publishing archival (move impression files to References/). Pavel often invokes only one stage at a time - phrases like "записал подкаст, подготовишь файл" or "положи на NAS" jump to Stage 6, "перемести в референсы" jumps to Stage 7. Don't run earlier stages in those cases; just go to the requested one.
