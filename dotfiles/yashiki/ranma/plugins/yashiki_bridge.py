...
DEFAULT_MODE_ICON = "rectangle.split.3x1"
...
def build_palette(accent_hex, fullscreen_hex):
    ...
    return {
        ...
        # Fullscreen tag (bottom indicator recolored, not the digit)
        "fullscreen": fullscreen_hex,
        "fullscreen_unfocused": hex_rgba(*parse_hex(fullscreen_hex), 128),
...
def load_config():
    ...
    defaults = {
        "accent_color": "#ffffffcc",
        "font_family": "Hack Nerd Font",
        "fullscreen_color": "#ff9f0a",
        "layout_icons": {"tatami": "rectangle.split.2x1", "byobu": "rectangle.stack"},
    }
    try:
...
def update_all(state):
    ...
    for slot in range(1, MAX_DISPLAYS + 1):
        ...
        display_fullscreen = fullscreen_per_display.get(did, 0)
        ...
        for tag_num in range(1, NUM_TAGS + 1):
            ...
            is_fullscreen = (display_fullscreen & bitmask) != 0
            ...
            if is_active:
                ...
                ranma_set(
                    f"{base}.ul.ind",
                    background_color=p["fullscreen"] if is_fullscreen else accent,
                    width=sz["ul_bar_w"],
                    height=sz["ul_bar_h"],
                    corner_radius="1",
                )
                ...
                ranma_set(
                    f"{base}.ul.ind",
                    background_color=(p["fullscreen"] if focused else p["fullscreen_unfocused"])
                    if is_fullscreen
                    else (p["dot_color"] if focused else p["dot_color_unfocused"]),
                    width=sz["dot_w"],
                    height=sz["dot_h"],
                    corner_radius="2",
                )
        ...
        if sz["show_mode"]:
            layout = state["layouts"].get(did)
            ranma_set(
                f"ws.d{slot}.mode.icon",
                icon=config["layout_icons"].get(layout, DEFAULT_MODE_ICON),
            )
...
def process_snapshot(event, state):
    ...
    default_layout = event.get("default_layout")
    state["layouts"] = {
        did: (query_layout(did) or default_layout) for did in state["displays"]
    }
...
def process_event(event, state):
    ...
    if t == "tags_changed":
    ...
    elif t == "layout_changed":
        state["layouts"][str(event["display_id"])] = event["layout"]
        ...
        d = event["display"]
        did = str(d["id"])
        state["displays"][did] = {"visible_tags": d["visible_tags"]}
        state["layouts"].setdefault(did, query_layout(did))
        if d.get("is_focused"):
            state["focused_display"] = did
        assign_slots(state)
        ensure_display_items(state)
    elif t == "display_removed":
        removed_id = str(event["display_id"])
        state["displays"].pop(removed_id, None)
        state["layouts"].pop(removed_id, None)
        ...
        try:
            proc = subprocess.Popen(
                [
                    ...
                    "--snapshot",
                    "--filter",
                    "tags,focus,window,display,layout",
                ],
            ...
            state = {
                ...
                "layouts": {},
                "config": config,
                "palette": build_palette(config["accent_color"], config["fullscreen_color"]),
            }
